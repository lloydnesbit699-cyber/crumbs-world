...
SCHEMA_VERSION = 2  # v5.29: object-layer cells may be {tid,size,hero,flavor,
                      # interactive,role} dicts (legacy int cells migrate on load),
                      # plus height_override + the .portals.json sidecar
...
def _store_custom_tile(name, preset, frame_ms, pil_images, scope="local",
    ...
    flavor: explicit world/decor override, else name-guessed.
...
def _find_spawn():
    ...
    # v5.29: spawn priority — per-instance hero flags first (first flag =
    # the PC), then the hero_tiles designation list (first designated hero
    # with a placed, walkable instance; the player's pick is never
    # overridden), then the legacy hero_tile for older maps, then center.
...
def _paint_value_safe(layer, tile_id, attrs=None):
    ...
    (size/hero/flavor/interactive/role); a fully-default instance stays the
    bare int. Non-dict attrs are ignored. Never raises."""
    try:
        v = _paint_value(layer, tile_id)
    except (TypeError, ValueError):
        return False, None
    if layer == "objects" and isinstance(attrs, dict) and v is not None:
        try:
            v = core.make_obj_cell(v, attrs)
        except Exception:
            pass
    return True, v
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _do_POST_impl(self, path):
        ...
        if path == "/api/recovery/reset":
            ...
            # report. Checkpoints and saves are untouched.
            # (do_POST already gated the write key above for non-local mode.)
            res = {"ok": True, "archived": None}
            if _recovery_store:
                res.update(_recovery_store.reset(
        ...
        if path == "/api/paint":
            ...
            # attrs stamps per-instance attributes on object placement
            ...
            if layer == "tiles" and tile_id == "natural":
                ...
                if not isinstance(attrs, dict):
                    attrs = None
                ok, new_val = _paint_value_safe(layer, tile_id, attrs)
                if not ok:
        ...
        if path == "/api/stroke":
            ...
            for c in cells:
                ...
                tid = c.get("tile_id")
                # v5.29: per-cell attrs fall back to the stroke-wide attrs;
                # one bad cell never kills the whole stroke.
                ...
                if layer == "tiles" and tid == "natural":
                    ...
                    ok, new_val = _paint_value_safe(layer, tid, cell_attrs)
                    if not ok:
                        continue
                grid = _grid(layer)
                if grid[y][x] != new_val:
